...
One transaction creates and publishes or extends the shared pure cue, applies both word plans,
retires explicitly selected overbroad shared targeted cues globally (preserving
...
cue membership is set-add only. Pure-cue content is shared; learner scheduling,
attempts, evidence, and compensation remain private. `proxied` is derived from
current shared production content, not written to learner relevance state.
...
| `add_contrast_candidate` | No equivalent operation; it created unresolved intake rather than trainable content | Retired live path and storage; the explicit SWI-47 upgrade accounts for and drops the legacy table |
...
| In-session or out-of-session bad production prompt reporting | No registered reflection operation | Live writes are retired; the SWI-47 upgrade preserves active intent as narrow prompt exclusions, not as a cue adapter |
