...
| `reflection-api.test.ts` | Generation statuses/errors, queue/detail, compact generation run log/retry, strict independent proposal review, quality tag upsert/stats, Help inbox Done, attention badges, explanation-only manual authorization, apply/withdraw, startup recovery | `server/index.ts` registered Express route handlers + temporary SQLite |
...
| `attention-badges.test.ts` | Unseen Help count, sticky `inbox_seen_at`, dismiss/reopen, What’s New cursor | `server/db/attention.ts` + temporary SQLite |
