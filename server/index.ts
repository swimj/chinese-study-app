...
export function createApp(options: CreateAppOptions = {}) {
  ...
  app.post('/api/reflection-generation-runs/:runId/upgrade-v1-and-retry', async (req, res) => {
    const runId = req.params.runId;
    if (typeof runId !== 'string' || runId.trim().length === 0) {
      res.status(400).json({ error: 'Expected non-empty reflection generation run id' });
      return;
    ...
    try {
      const result = await reflectionGenerationService.retryUpgradedV1(runId.trim());
      res.status(result.status === 'created' ? 201 : 200).json(result);
    } catch (error) {
      if (isReflectionNotFoundError(error, 'Reflection generation run not found.')) {
        res.status(404).json({ error: 'Reflection generation run not found' });
      ...
      res.status(500).json({ error: 'Failed to upgrade and retry reflection generation' });
    }
  });
