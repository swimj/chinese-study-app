...
import type { ReflectionProviderFixtureV0, SessionReflectionResultV3 } from '../contracts.js';
...
export async function runFixture(options: RunFixtureOptions): Promise<ReflectionRunArtifactV0> {
  ...
  let parsedResult: SessionReflectionResultV3 | null = null;
    ...
    if (isOutputTruncationFinishReason(providerResult.finishReason)) {
      ...
      if (status !== 'invalid_json') {
        ...
        if (validationErrors.length > 0) {
          ...
          parsedResult = parsed as SessionReflectionResultV3;
