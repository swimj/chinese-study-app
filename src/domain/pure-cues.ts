...
export type PureCueAcceptedAnswer = {
  wordId: string;
  hanzi: string;
...
export function resolvePureCueResponse(
  ...
  if (response === null) return { outcome: 'rejected', submittedWordId: null };
  const normalizedResponse = normalizeProductionAnswerForProfile(response, profileId);
  const match = snapshot.acceptedAnswers.find((answer) => (
    [answer.hanzi, answer.traditional]
      .filter((form): form is string => form !== null)
      .some((form) => normalizeProductionAnswerForProfile(form, profileId) === normalizedResponse)
  ));
  return match
