  ...
  return {
    ...
    preparePromotion(input) {
      ...
      const items = current.diagnosisBundle.items.map((item) => ({
        ...item,
        promotionEvidence: testPromotionEvidence(item, resultByItemId.get(item.itemId)!),
      }));
      ...
      const promotionBundle = {
        ...
        items: items.flatMap((item) => {
          ...
          return [{
            ...
            learnerExplanation: result.learnerExplanation,
...
function testPromotionEvidence(
  ...
  result: { diagnosisTags: string[]; proposals: Array<{ operation: unknown }> },
  ...
  const diagnosisSelected = result.diagnosisTags.some((tag) => (
    tag === 'valid_or_near_valid_alternate'
    || tag === 'cue_overlap_hides_usage_difference'
    || tag === 'production_cue_overloaded'
  ));
  const proposesMultiAnswerCue = result.proposals.some(({ operation }) => {
    if (!isRecord(operation) || operation.kind !== 'repair_production_cue' || operation.version !== 2) {
      return false;
    }
    if (!Array.isArray(operation.changes)) return false;
    return operation.changes.some((change) => {
      ...
      return drafts.some((draft) => (
        ...
        && draft.acceptedWordIds.length > 1
      ));
    });
  });
  if (!diagnosisSelected && !proposesMultiAnswerCue) return null;
  return {
    diagnosisTags: result.diagnosisTags as PureCuePromotionEvidenceV1['diagnosisTags'],
