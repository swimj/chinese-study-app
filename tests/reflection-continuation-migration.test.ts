...
test('continuation migration preserves the previous schema rows and is repeatable', () => {
  ...
  try {
    ...
    try {
      const migrationIndex = schemaMigrations.findIndex((migration) =>
        migration.id === 'app_schema:0011_reflection_generation_continuations');
      assert.ok(migrationIndex >= 0);
      const previousMigrations = schemaMigrations.slice(0, migrationIndex);
