...
describe('reflection comparison-arm registry', () => {
  test('registers the required dogfood arms without enabling new egress by default', () => {
    ...
    assert.equal(REFLECTION_MODEL_ARMS.filter((arm) => arm.enabledByDefault).length, 3);
  });
