...
describe('reflection comparison-arm registry', () => {
  ...
  test('uses the shared 15-minute provider timeout for every registered arm', () => {
    ...
    assert.equal(GLM_REFLECTION_MODEL_CONFIG.maxOutputTokens, 131_072);
    assert.equal(GLM_FLASH_HIGH_REFLECTION_MODEL_CONFIG.maxOutputTokens, 131_072);
    ...
    for (const arm of REFLECTION_MODEL_ARMS) {
      ...
      assert.equal(
        ...
        arm.choice.startsWith('zai:') ? 131_072 : 50_000,
        arm.choice,
      );
    }
  });
