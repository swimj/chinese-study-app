...
describe('stash ratio setting', () => {
  test('a stored ratio shifts the split', () => {
    assert.deepEqual(splitRemainingUnstudiedQuota(10, 'mixed', 0.5), { stashSlots: 5, dietSlots: 5 });
    assert.deepEqual(splitRemainingUnstudiedQuota(10, 'mixed', 0.8), { stashSlots: 8, dietSlots: 2 });
    assert.deepEqual(splitRemainingUnstudiedQuota(10, 'mixed', 0), { stashSlots: 0, dietSlots: 10 });
    assert.deepEqual(splitRemainingUnstudiedQuota(10, 'mixed', 1), { stashSlots: 10, dietSlots: 0 });
    assert.deepEqual(splitRemainingUnstudiedQuota(5, 'mixed', 0.5), { stashSlots: 2, dietSlots: 3 });
  });

  test('rejects out-of-range ratios', () => {
    assert.throws(() => splitRemainingUnstudiedQuota(10, 'mixed', 1.5), /\[0, 1\]/);
    assert.throws(() => splitRemainingUnstudiedQuota(10, 'mixed', Number.NaN), /\[0, 1\]/);
  });
