...
describe('word meanings', { concurrency: false }, () => {
  ...
  test('migrates legacy words into normalized word_meanings rows on startup', async () => {
    const legacyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'chinese-study-app-legacy-word-meanings-'));
    const legacyDbPath = path.join(legacyDir, 'app.db');
    const legacyWordId = 'legacy-word';
    ...
    try {
      ...
      const moduleUrl = `${pathToFileURL(path.resolve('server/db.ts')).href}?test=legacy-${Date.now()}`;
      const legacyModule = await import(moduleUrl);
      const migratedMeanings = legacyModule.getWordMeanings(legacyWordId);

      assert.deepEqual(migratedMeanings.map((meaning) => meaning.text), ['good', 'well']);
      assert.deepEqual(migratedMeanings.map((meaning) => meaning.position), [0, 1]);
      assert(migratedMeanings.every((meaning) => meaning.showOnProductionPrompt));
    } finally {
      if (previousMode === undefined) {
        delete process.env.APP_MODE;
      } else {
